...
const FormField = (props: FormFieldProps) => {
  ...
  return (
    <div className={`flex flex-col ${disabled === true ? 'form-field--disabled' : ''} ${className ?? ''}`}>
      {children}
    </div>
  )
}
...
const Label = (props: FormLabelProps) => {
  ...
  return (
    <label className={formFieldLabel({ class: className ?? '' })} {...rest}>
      {hasError && <ExclamationCircleIcon className="w-5 self-center animate-appear text-negative-10 mie-1" />}
      {children}
    </label>
  )
}
...
const HelpBlock = (props: HelpBlockProps) => {
  ...
  return (
    <p className={`${hasError === true ? 'mt-1 text-xs text-negative-11' : 'sr-only'} ${className}`} {...rest}>
      {children}
    </p>
  )
}

...
interface ListFiltersProps {
  ...
  setFilterNameRally: (name: string) => void
  filterNameRally: string
  filterCreatorEthAddress: string
  setFilterCreatorEthAddress: (value: string) => void
}
...
export const ListFilters = (props: ListFiltersProps) => {
  const {
    ...
    setFilterNameRally,
    filterNameRally,
    filterCreatorEthAddress,
    setFilterCreatorEthAddress,
  } = props
  const queryListInterests = useGetProfilesInterests()
  const [inputNameRally, setInputNameRally] = useState(filterNameRally)

  useDebouncedEffect(
    () => {
      setFilterNameRally(inputNameRally)
    },
    [inputNameRally],
    500,
    1000,
  ...
  return (
    <>
      <div className="w-full pb-6">
        <label className="font-bold pb-1 block">Rally name must includes</label>
        <FormInput
          value={inputNameRally}
          onChange={(e) => setInputNameRally(e.currentTarget.value)}
          className="w-full"
          placeholder="Eg: Lensfluencer"
          type="text"
          hasError={false}
        />
      </div>
      <div className="w-full pb-6">
        <label className="font-bold pb-1 block">Rally creator Ethereum address</label>
        <div className="relative z-20">
          <LensProfileSuggestions
            onSelectValue={(value: string) => {
              setFilterCreatorEthAddress(value)
            }}
          />
          <FormInput
            onChange={(e) => setFilterCreatorEthAddress(e.currentTarget.value)}
            type="text"
            value={filterCreatorEthAddress}
      ...
      <Listbox value={filterStatuses} onChange={setStatusesFilter} multiple horizontal>
        ...
        <Listbox.Options static={true} className="mt-4 grid gap-3 grid-cols-2 2xs:grid-cols-3">
          {[
            ...
            <Listbox.Option
              ...
              key={`upcoming-status-${status.value}`}
              value={status.value}
            >
      ...
      <div className="py-6 border-t border-neutral-3 mt-8 space-y-4">
        ...
        {queryListInterests?.data
          ...
          .map((label) => (
            <div key={`filter-upcoming-category-${label}`}>
              <Listbox value={filterCategories} onChange={setCategoriesFilter} multiple horizontal>
                ...
                <Listbox.Options className="text-neutral-12 pt-2 flex flex-wrap gap-3" static={true}>
                  <Listbox.Option
                    className={button({
                      class: `${filterCategories?.includes(label) ? '!pis-1ex' : ''} !tracking-none space-i-1ex`,
                      scale: 'xs',
                      intent: filterCategories?.includes(label) ? 'neutral-on-dark-layer' : 'neutral-outline',
                    })}
                    value={label}
                  >
                    {filterCategories?.includes(label) && <CheckCircleIcon className="text-interactive-9 w-5" />}
                    {/* @ts-ignore */}
                  ...
                  {queryListInterests?.data
                    ...
                    .map((interest) => (
                      <>
                        <Listbox.Option
                          key={`upcoming-filter-subcategory-${interest}`}
                          className={button({
                            scale: 'xs',
                            intent: filterCategories?.includes(interest) ? 'neutral-on-dark-layer' : 'neutral-outline',
                            class: `${
                              filterCategories?.includes(interest) ? '!pis-1ex' : ''
                            } !tracking-none space-i-1ex`,
                          })}
                          value={interest}
                        >
                          {filterCategories?.includes(interest) && (
